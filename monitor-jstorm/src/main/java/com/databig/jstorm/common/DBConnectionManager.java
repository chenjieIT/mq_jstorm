...
public class DBConnectionManager {
	...
	private void createPools(Properties props) {
		...
		while (propNames.hasMoreElements()) {
			...
			if (name.endsWith(".url")) {
				...
				DBConnectionPool pool = new DBConnectionPool(poolName, url, user, password, max, min, logInterval);
	...
	class DBConnectionPool {
		...
		/**
		 * 创建新的连接池连；接池始终保持一定数量的连接.当申请连接时如果连接池中连接数小于定值 则申请新的连接
		 ...
		 *            最小链接数
		 */
		public DBConnectionPool(String name, String URL, String user, String password, int maxConn, int minConn,
				long logInterval) {
			this.name = name;
			this.URL = URL;
			this.user = user;
		...
		private void monitor() {
			while (true) {
				...
				if ((System.currentTimeMillis() - logTimer) > logInterval) {
					Enumeration<Connection> checkconn = freeConnections.elements();
